...
@Component({
  ...
  template: `
    ...
    <article class="card" *ngFor="let r of sortedResults(); trackBy: trackRes">
      <header class="head">
        <div class="badges">
          <span class="chip" [class.green]="!r.flight2">
            {{ r.flight2 ? '1 scalo' : 'Diretto' }}
          </span>
        ...
        <ng-container *ngFor="let s of segmentsOf(r); let i = index; let last = last">
          <div class="segment">
            ...
            <div class="route">
              ...
              <div class="meta">
                {{ s.duration | duration }} · {{ s.code || '' }}
              </div>
            </div>

            <div class="seg-tickets">
              <ng-container *ngFor="let t of tickets(r, s.key)">
          ...
          <div class="layover" *ngIf="!last && r.flight2">
            <mat-icon>schedule</mat-icon>
            Scalo a {{ s.to_ }} ·
            {{ layoverMinutes(s, segmentsOf(r)[i + 1]) | duration }}
          </div>
        </ng-container>
      </section>
      ...
      <footer class="foot">
        <div class="total">
          <span *ngIf="totalPrice(r) as tp">
            Totale: <strong>€ {{ tp | number:'1.0-0' }}</strong>
          </span>
        </div>
        <div class="actions">
          <button
  ...
  styles: [`
    ...
    .card { background:#fff; border-radius:18px; box-shadow:0 12px 30px rgba(0,0,0,.06); padding:16px; margin-bottom:16px }
    .head { display:flex; align-items:center; justify-content:space-between; margin-bottom:8px }
    .carrier-name { font-weight:600; font-size:1.05rem; }
    .carrier small { margin-left:6px; opacity:.7; font-weight:400; }
    .carrier-logo { margin-top:10px; height:22px; width:auto; object-fit:contain; filter: drop-shadow(0 1px 0 rgba(0,0,0,.04)); }
    ...
    .segment { position: relative; display:grid; grid-template-columns: 180px 1fr 180px; gap:16px; align-items:center; padding:14px 12px; border-radius:14px; background:#f8fafc; }
    .when { display:grid; grid-template-areas: "dow" "day" "month" "year" "hour"; gap:2px; text-transform:capitalize }
    ...
    .seg-price { position:absolute; right:12px; bottom:10px; font-weight:700; color:#0f172a; background:#ffffffc7; padding:4px 8px; border-radius:10px; box-shadow:0 2px 8px rgba(0,0,0,.06); }
    ...
    .cta { appearance:none; border:0; cursor:pointer; border-radius:999px; padding:10px 18px; font-weight:600; transition: filter .15s ease, transform .02s ease; }
...
export class ResultsListComponent {
  ...
  readonly SEG_KEYS = ['flight1', 'flight2'] as const;
  private readonly FALLBACK_LOGO = '';
  private base = environment.apiBase;

  constructor(private http: HttpClient) {}

  segmentsOf(r: FlightResult): any[] {
    const segs: any[] = [];
    if (r.flight1) segs.push({ ...r.flight1, key: 'flight1' });
    if (r.flight2) segs.push({ ...r.flight2, key: 'flight2' });
    return segs;
}
  ...
  hasTickets(r: FlightResult, key: 'flight1' | 'flight2'): boolean {
    const list = r.matchedTicketsBySegment?.[key] ?? [];
    return list.length > 0;
  }

  tickets(r: FlightResult, key: 'flight1' | 'flight2'): TicketDTO[] {
    return r.matchedTicketsBySegment?.[key] ?? [];
  }
  ...
  trackRes(_: number, r: any) {
    return r.flight1?.id + '-' + (r.flight2?.id ?? '0');
  }

  buildPurchaseFlight(r: any): FlightResult {
    const segs = this.segmentsOf(r);
    const first = segs[0];
    const last = segs[segs.length - 1];
    const route = r.route ?? { from_: first?.from_, to_: last?.to_ };
    const departure = r.departure ?? first?.departure ?? null;
    const arrival = r.arrival ?? last?.arrival ?? null;

    const totDuration = r.tot_duration
  ...
  sortedResults(): any[] {
    ...
    arr.sort((a, b) => {
      const pa = this.totalPrice(a) ?? Infinity;
      const pb = this.totalPrice(b) ?? Infinity;
  ...
  logoOf(r: any): string {
    return r?.airline?.logo?.trim?.() || this.FALLBACK_LOGO;
  }

  onLogoError(ev: Event) {
    const img = ev.target as HTMLImageElement;
    if (img && img.src !== this.FALLBACK_LOGO) img.src = this.FALLBACK_LOGO;
